...
Bottom bar (edit)    Delete (casper text button, left; becomes **Cancel** — discard + pop — once the form is dirty)  ·  Save (blue, right)   — same row style as Create/Edit Wallet
```
...
- Bottom bar: **Delete** (casper; confirm dialog "Delete this transaction?") → soft delete → pop. While `isDirty`, the left slot shows **Cancel** instead (pop without saving). **Save** → `repo.update` (diffs tags; recomputes conversion only if amount or currency changed, fetching a fresh rate and showing the hint before the user taps Save) → pop.
...
### 10.1 Tokens (`UIColorToken`, `UITypographyToken`)

Typography is themed like nour's design system: `AppTheme.of(context).typo` (`context.typo`) exposes `inter.*` / `montserrat.*` styles with the text colour already baked in per theme (`InterStyle.light()` = bismark / casper, `InterStyle.dark()` = bismarkLight / casper). Use `copyWith` only for semantic accents (blue, expense, destructive). Icons default to `colors.iconColor` = bismark in both themes.

| Role | Light | Dark (2026-09-03 pass) |
|---|---|---|
| Background | `cararra #FAFAF8` | `#120F1B` |
| Surface (tiles, sheets) | `white` | `#15121F` |
| Text primary | `bismark #456285` | `bismarkLight #D6E2F0` |
| Text secondary / placeholders | `casper #AEC2D4` | `casper #AEC2D4` |
| Divider / outlines (`dividerColor`) | casper 30 % | `#191524` |
| Numpad keys (`numpadKeyColor`) | white 50 % | `#191524` 50 % |
| Icons (`iconColor`) | `bismark` | `bismark` |
| Bottom-bar Delete / Cancel (`UiTextButton.secondary`) | `casper` | `casper` |
...
| 13 | `ui_color_token.dart` | ~~dark theme reuses light `bismark`/`casper`~~ | Done 2026-09-03 — see 10.1. |
